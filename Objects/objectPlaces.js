import { React } from 'react';


export const places = [
    {
    coordinate: {
        latitude: 24.03195663600582, 
        longitude: -104.67376510853963
    },
    photo: "https://streetviewpixels-pa.googleapis.com/v1/thumbnail?panoid=TzDU2ZgaTirezME4QZzSVg&cb_client=search.gws-prod.gps&w=408&h=240&yaw=304.3983&pitch=0&thumbfov=100",
    instituto: " Instituto Duranguense de la Juventud ",
    domicilio: " Calle Gomez Palacio #934, entre Hidalgo y Zaragoza. ",
    codigoPostal: 34000, 
    telefono: 6184962696,
    horarios: " Lunes a Domingo. 08:00 am - 19:00 pm ",
    servicios: [" Apoyos sociales", " Culturales", " Educativos a la juventud. "],
    costos: " Gratuito ",
    requisitosAtencion: " Acudir directamente o mediante llamada telefonica. ",
},
{
    coordinate: {
        latitude: -25.436658952319963,
        longitude: -49.28039871579469,
    },
    photo: "https://streetviewpixels-pa.googleapis.com/v1/thumbnail?panoid=aPlp5wr1mR-Y5HGpJ9VDPA&cb_client=search.gws-prod.gps&w=408&h=240&yaw=3.1910462&pitch=0&thumbfov=100",
    instituto: " Centro de Psicoterapia Familiar e Individual ",
    domicilio: " Blv. Jose Maria Patoni, Manzana #105, Predio Rustico La Tinaja y Los Lugos. ",
    codigoPostal: 34217,
    telefono: 6181379101,
    horarios: " Lunes, martes, jueves, viernes y sabado. 16:00 pm - 16:45 pm ",
    servicios: [" Terapia psicologica individual ", " familiar y de pareja ", " dictamenes psicologicos. "],
    costos: "$50 por terapia individual; $100 de terapia de pareja y valoraciones. ",
    requisitosAtencion: " Acudir directamente o mediante llamada telefonica. ",
},
{
    coordinate: {
        latitude: 23.997449720052934, 
        longitude: -104.67612405898662
    },
    photo: "https://lh3.googleusercontent.com/gps-proxy/ALd4DhEiLP6mcr356wqT0LcSSWRiLk2jKPPLyiedDH37_XJyoD0d_aVuRPXoflm2WXlu77JMbcna_7mRJvJMFoVIqfBoaw3TF0tr1YKNI3abGGAXnVNo6WLmYL_ZB9MIKm1Rzft1S9i8OMCq5ltIev49k8PVCB0RZZmVQe4RhomHVdr_LxIKv3ciFbyjZu5hslEzxIjcdLI=w427-h240-k-no",
    instituto: " CESSA 450 (Centro de Salud con Servicios Ampliados).  ",
    domicilio: " Toma de Zacatecas N. 129, Col. Division Del Norte (Division Del Norte) ",
    codigoPostal: 34000,
    telefono: 6181377364,
    horarios: " Lunes a Domingo. 08:00 - 14:30 pm ",
    servicios: [" Tratamiento psicologico para adolescentes y adultos. "],
    costos: " Consulta psicologica: $53 / Gratuita con seguro popular. ",
    requisitosAtencion: " El paciente puede acudir directamente directamente a solicitar la consulta, \ ser referido por alguna institucion externa o bien de la misma secretaria de salud. ",
},
{
    coordinate: {
        latitude: 24.029883058669277, 
        longitude: -104.60383768966523
    },
    photo: "https://streetviewpixels-pa.googleapis.com/v1/thumbnail?panoid=srca6XM8kIJRftBvrlStPg&cb_client=search.gws-prod.gps&w=408&h=240&yaw=6.7662673&pitch=0&thumbfov=100",
    instituto: " CIJ (Centro de Integracion Juvenil) ",
    domicilio: " Carretera Mexico Km 3.5 Mz. A Lote 8 Col. Industrial ",
    codigoPostal: 34000,
    telefono: 6188130932,
    horarios: " Lunes a Viernes 08:00 am - 19:30 pm ",
    servicios: [" Tratamiento y prevencion contra adicciones. "],
    costos: " Primera consulta: $70 posterior cuota de recuperacion valorada por Trabajo Social. ",
    requisitosAtencion: " Unicamente se solicita que el paciente ingrese de manera voluntaria. ",
},
{
    coordinate: {
        latitude: 24.046446919923543, 
        longitude: -104.59385274733452
    },
    photo: "https://lh5.googleusercontent.com/p/AF1QipMf2HS9LjdD238I8HJ0QXqVv_aJWrL07S_f37qK=w426-h240-k-no",
    instituto: " Hospital de Salud Mental Dr. Miguel Vallebueno ",
    domicilio: " Av. Fidel VElazquez Sanchez s/n, Agricola Veinte de Noviembre ",
    codigoPostal: 34234,
    telefono: 6184550633,
    horarios: " Lunes a Sabado las 24 horas diarias ",
    servicios: [" Area de urgencias ", " Servicios de psicologia ", " Psiquiatria mediante citas "],
    costos: " Primera vez: $105 / Subsecuentes: $86 ",
    requisitosAtencion: " Referencia externa o Institucion de Salud. Si se trata de una emergencia, se acude directamente al area de urgencias.  ",
}]